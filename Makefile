...
help:
	...
	@echo "  make stage-app APP=DSperate-pak DEVICE=mlp1 explicitly stage the optional DSperate content pak"
	...
	@echo "  make log-safety-policy-test               test the MLP1 emulator log-safety gates"
...
release-preflight:
	@DEVICE="$(DEVICE)" LEAF_WORKSPACE_DIR="$(WORKSPACE_DIR)" \
		TOOLCHAIN_IMAGE="$(TOOLCHAIN_IMAGE)" TOOLCHAIN_REPO="$(TOOLCHAIN_DIR)" \
		LEAF_RELEASE_CHANNEL="$(LEAF_RELEASE_CHANNEL)" \
		REBUILD_CORES="$(REBUILD_CORES)" FORCE_REBUILD_CORES="$(FORCE_REBUILD_CORES)" \
...
log-safety-policy-test:
	python3 scripts/validate-logsafe-wrappers-test.py
