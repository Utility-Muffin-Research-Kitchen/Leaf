...
cd "$STATE_ROOT"
# LOG-SAFE-1. The DraStic log lives on the same FAT card as the session log, so
...
if printf '\n' >>"$LOG_ROOT/drastic.log" 2>/dev/null; then
    exec >>"$LOG_ROOT/drastic.log" 2>&1
else
    exec >/dev/null 2>&1
fi
exec "$DRASTIC_BIN" "$ROM_PATH"
EOF
    chmod 755 "$path"
}
